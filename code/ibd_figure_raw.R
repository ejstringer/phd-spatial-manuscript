...
imgph <- png::readPNG("../data/pics/pherm_close.png")

imgsy <- png::readPNG("../data/pics/syoung_close.png")
...
fig1 <- grid.arrange(l2,
                     ...
                     con + #labs(tag = 'A)') +
                       theme(
                         legend.key.size = unit(0.40, units = 'cm'),
                             plot.margin = margin(0, 0, 0, 0, "cm")),
                     p + #labs(tag = 'B)') + 
                       theme(legend.position = 'none'),
                     nrow = 2, layout_matrix = lay)
ggsave('./figures/fig1_captures_phases_npp_ph.png', plot = fig1,
       height = 14, width = 20, units = 'cm', dpi = 600)
